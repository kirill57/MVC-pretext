...
**Implemented locally following the user's subsequent authorization.** The first-year design below follows [MVC-pretext-first-year-repair-addendum.md](../../MVC-pretext-first-year-repair-addendum.md). Optional material is marked and folded. See the [implementation and validation report](../first-year-repair/implementation-report.md) for changes, exercise checks, source relocations and build results. The [approved planning version](repair-plan-first-year-approved.md) is preserved; the design specification below is retained as the implementation contract.
